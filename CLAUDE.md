...
```bash
curl -s http://127.0.0.1:48888/api/tasks/N
```
...
```bash
curl -s -X POST http://127.0.0.1:48888/api/tasks/N/result \
...
- 필요하면 자기 worker에게 task를 만든다: `POST http://127.0.0.1:48888/api/inbox/<inboxId>/tasks` 에 `{"toAgentId": <workerId>, "parentTaskId": N, ...}`.
